...
int MainNode::run()
{
    ...
    // publish waypoints to move_base
    while (navigation_state){
        // publish waypoint
        ...
        // if queue empty, stop navigation
        if (waypoint_queue_.size() == 0){
            navigation_state = false;
        }
        ros::spinOnce();
    }
    ...
    while (ros::ok()){
        // run timers to update pose etc.

        //get distance to current goal
        currentPoseLoop();

        if (distanceToGoal < 0.5 && waypoint_queue_.size() > 0){
            waypoint_queue_.erase(waypoint_queue_.begin());
            ROS_INFO("Goal reached");
            break;
        }
...
public:
    WaypointManagerNode() : nh_("~") {
        map_frame_ = "map";
        base_link_frame_ = "base_link";
        nh_.param<std::string>("waypoint_file_path", waypoint_file_path_, "./scripts/waypoints.csv");

        save_waypoint_sub_ = nh_.subscribe("/save_waypoint", 10, &WaypointManagerNode::saveWaypointCallback, this);
        start_nav_sub_ = nh_.subscribe("/start_nav", 10, &WaypointManagerNode::startNavigationCallback, this);
    }
    ...
    void saveWaypointCallback(const std_msgs::Empty::ConstPtr& msg) {
        ...
        // Append the current pose to the waypoint queue
        waypoint_queue_.push_back(current_pose);

        // Save the waypoints to file
        std::ofstream ofs(waypoint_file_path_, std::ios::out | std::ios::trunc);
        if (ofs.is_open()) {
            for (auto waypoint : waypoint_queue_) {
                ofs << waypoint.pose.position.x << "," << waypoint.pose.position.y << "," << waypoint.pose.orientation.z << "," << waypoint.pose.orientation.w << std::endl;
            }
            ROS_INFO_STREAM("Saved " << waypoint_queue_.size() << " waypoints to file " << waypoint_file_path_);
            ofs.close();
        } else {
            ROS_WARN_STREAM("Failed to open file " << waypoint_file_path_ << " for writing");
        }
    }

    void startNavigationCallback(const std_msgs::Empty::ConstPtr& msg) {
        ...
        if (ifs.is_open()) {
            ...
            while (std::getline(ifs, line)) {
                ...
                iss >> waypoint.pose.position.x >> waypoint.pose.position.y >> waypoint.pose.orientation.z >> waypoint.pose.orientation.w;
        ...
        // Send the waypoint goals to move_base
        MoveBaseClient ac("move_base", true);
        while (!ac.waitForServer(ros::Duration(5.0))) {
            ROS_INFO("Waiting for the move_base action server to come up");
        }
    ...
    ROS_INFO_STREAM("Sending goal: " << goal.target_pose.pose.position.x << ", " << goal.target_pose.pose.position.y);

    // send the goal and wait for completion
    ac_.sendGoal(goal);
    //ac_.waitForResult();
    // distance between goal pose and current pose
    difference = this->current_pose.inverseTimes(goal) 

    while (squrt((difference.pose.x)^2+(difference.pose.y)^2) > 2){
        //update difference and wait
        difference = this->current_pose.inverseTimes(goal) 
    ...
    // check if there are remaining waypoints in the queue
    if (!waypoint_queue_.empty()) {
        // check the distance to the next waypoint
        geometry_msgs::PoseStamped current_pose;
        current_pose = getCurrentPose();
        double distance_to_next = distance(current_pose.pose.position, waypoint_queue_.front().pose.position);
        if (distance_to_next < 2.0) {
            // pop the front waypoint and continue to the next
            ROS_INFO_STREAM("Next goal is within 2m, moving to the next waypoint...");
            waypoint_queue_.pop();
        }
    }
    }
...
int main(int argc, char **argv) {
  ...
  // Create instance of MyNode class
  MyNode my_node(nh);
